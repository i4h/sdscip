...
#include "xml/xml.h"
#include <regex.h>
#define READER_NAME             "osilreaderControl"
#define READER_DESC             "file reader for Control problems in OS instance language (OSiL) format"
#define READER_EXTENSION        "osilc"
   ...
   )
{
   ...
   for( soscons = xmlFirstChild(soscons); soscons != NULL; soscons = xmlNextSibl(soscons), ++count )
   {
      ...
      if( *attrval != '\0' || (sosorder != 1 && sosorder != 2 ) )
      {
      ...
      for( sosvar = xmlFirstChild(soscons); sosvar!= NULL; sosvar = xmlNextSibl(sosvar), ++varcount )
      {
         /* get variable id*/
          ...
          if (varcount == 0)
          {
             ...
             int reti;
             regmatch_t m[10];
             const char * to_match = SCIPvarGetName(vars[idx]);
             const char * p = to_match;
             char timeString[10];
             char levelString[10];
             int nomatch = regexec (&regex, p, 10, m, 0);

             SCIPdbgMsg("Getting name of first variable %s and getting time\n",SCIPvarGetName(vars[idx]));

             /* Compile regular expression */
             reti = regcomp(&regex, "[a-zA-Z0-9_-]*_([0-9]+?)_8\\(([0-9]+)(,[0-9]+)*\\)", REG_EXTENDED);
             ...
             if (!nomatch) {
                for (i = 0; i < 3; i++) {
                   ...
                   if (i == 1)
                   {
                      sprintf(levelString,"%.*s", (finish - start), to_match + start);
                      SCIPdbgMsg("found level %s\n",levelString);
                   }
                   else if (i == 2)
                   {
                      sprintf(timeString,"%.*s", (finish - start), to_match + start);
                      SCIPdbgMsg("found time %s\n",timeString);
                   }
                }
                p += m[0].rm_eo;
             }
             ...
             SCIPdbgMsg("time is '%s'\n",timeString);
             (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "eq_SOS%dn%d_%s_8_sos(%s)", type, count,levelString,timeString);
             ...
             SCIPdbgMsg("created constraint %s\n",name);
          ...
          {
          case 1:
             SCIP_CALL( SCIPaddVarSOS1(scip, cons, vars[idx], nsosvars - varcount) );
             break;
          case 2:
             SCIP_CALL( SCIPaddVarSOS2(scip, cons, vars[idx], nsosvars - varcount) );
             break;
          default:
             SCIPerrorMessage("unknown SOS type: <%d>\n", type); /* should not happen */
             SCIPABORT();
...
static
SCIP_DECL_READERCOPY(readerCopyOsilC)
{  /*lint --e{715}*/
   assert(scip != NULL);
...
static
SCIP_DECL_READERREAD(readerReadOsilC)
{  /*lint --e{715}*/
   const char* name;
   XML_NODE* start;
   ...
   )
{
   ...
   SCIP_CALL( SCIPsetReaderCopy(scip, reader, readerCopyOsilC) );
   SCIP_CALL( SCIPsetReaderRead(scip, reader, readerReadOsilC) );
